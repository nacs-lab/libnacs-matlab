...
classdef SeqContext < handle
    ...
    methods(Access=?SeqVal)
        ...
        function res = serializeArg(self, arg)
            if islogical(arg)
                assert(isscalar(arg));
                res = [int8(SeqVal.ArgConstBool), int8(arg)];
                return;
            elseif isinteger(arg)
                assert(isscalar(arg));
                res = [int8(SeqVal.ArgConstInt32), typecast(int32(arg), 'int8')];
                return;
            elseif isnumeric(arg)
                assert(isscalar(arg));
                res = [int8(SeqVal.ArgConstFloat64), typecast(double(arg), 'int8')];
                return;
            elseif isa(arg, 'SeqVal')
                head = arg.head;
                if head == SeqVal.HArg
                    res = [int8(SeqVal.ArgArg), typecast(uint32(arg.args{1}), 'int8')];
                    return;
                elseif head == SeqVal.HMeasure
                    res = [int8(SeqVal.ArgMeasure), typecast(uint32(arg.args{1}), 'int8')];
                    return;
                elseif head == SeqVal.HGlobal
                    res = [int8(SeqVal.ArgGlobal), typecast(uint32(arg.args{1}), 'int8')];
                    return;
                end
                res = [int8(SeqVal.ArgNode), typecast(uint32(getNodeID(self, arg)), 'int8')];
            else
        ...
        function ensureSerialize(self, val)
            ...
            if head == SeqVal.HArg
                ...
                serial = [int8(SeqVal.OPIdentity), int8(SeqVal.ArgArg), ...
                          typecast(uint32(argi), 'int8')];
            ...
            elseif head == SeqVal.HMeasure
                ...
                node_id = length(self.node_serialized) + 1;
                val.node_id = uint32(node_id);
                serial = [int8(SeqVal.OPIdentity), int8(SeqVal.ArgMeasure), ...
                          typecast(uint32(measure_id), 'int8')];
                self.node_serialized{node_id} = serial;
                return;
            elseif head == SeqVal.HGlobal
                ...
                node_id = length(self.node_serialized) + 1;
                val.node_id = uint32(node_id);
                serial = [int8(SeqVal.OPIdentity), int8(SeqVal.ArgGlobal), ...
                          typecast(uint32(global_id), 'int8')];
                self.node_serialized{node_id} = serial;
                return;
            elseif head == SeqVal.OPInterp
                ...
                for i = 1:3
                    arg = val.args{i};
                    serial = [serial, serializeArg(self, arg)];
                end
    ...
    methods
        ...
        function res = getValID(self, val)
            if islogical(val)
                ...
                serial = [int8(SeqVal.OPIdentity), int8(SeqVal.ArgConstBool), val];
            ...
            elseif isinteger(val)
                ...
                if isempty(val_id)
                    ...
                    serial = [int8(SeqVal.OPIdentity), int8(SeqVal.ArgConstInt32), ...
                              typecast(val, 'int8')];
                    self.node_serialized{val_id} = serial;
                end
                res = uint32(val_id);
            ...
            elseif isnumeric(val)
                ...
                if isempty(val_id)
                    ...
                    serial = [int8(SeqVal.OPIdentity), int8(SeqVal.ArgConstFloat64), ...
                              typecast(val, 'int8')];
                    self.node_serialized{val_id} = serial;
                end
                res = uint32(val_id);
    ...
    methods(Access=private)
        ...
        function res = getNodeID(self, val)
            if val.node_id == 0
                ensureSerialize(self, val);
            end
            res = val.node_id;
