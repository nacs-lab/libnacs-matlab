%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [output, status] = urlreadpost(urlChar, params, fname)
  ...
  %%                   {'file', dd, 'version', '3', 'api_key', 'API-KEY', ...
  %%                    'wait', 'Y'}, fname);
  ...
  if ~ischar(urlChar)
    error('MATLAB:urlreadpost:InvalidInput', ...
          'The first input, the URL, must be a character array.');
  end
  ...
  printStream.print(['Content-Type: multipart/form-data; boundary=', ...
                     boundary, eol, eol]);
  ...
  try
    ...
    output = native2unicode(typecast(byteArrayOutputStream.toByteArray', ...
                                     'uint8'), 'UTF-8');
  catch
    if catchErrors,
      return;
    else
      error('MATLAB:urlreadpost:ConnectionFailed', ...
