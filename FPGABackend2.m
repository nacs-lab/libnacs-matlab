...
classdef FPGABackend2 < PulseBackend
    ...
    methods
        function self = FPGABackend2(seq)
            self = self@PulseBackend(seq);
            % The FPGA poster is stateless so it is shared between all `FPGABackend2`s
            self.poster = FPGAPoster2.get(seq.config.fpgaUrls('FPGA1'));
            if isKey(seq.config.fpgaTTLWarningMask, 'FPGA1')
                self.ttl_warn_mask = int64(seq.config.fpgaTTLWarningMask('FPGA1'));
            end
        end
        ...
        function run(self)
            [self.seq_id, ttl_ovr, dds_ovr] = post(self.poster, self.req);
            if all(self.seq_id.uint8 == 0)
